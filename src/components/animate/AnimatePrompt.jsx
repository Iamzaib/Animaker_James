...
// import { animakerAction } from '../../store/actions/animakerAction';
...
//     establishWebSocketConnection(id)
//   },[])

//   const establishWebSocketConnection = (id) => {

//     const ws = new WebSocket(`ws://192.168.18.84:8188/ws?clientId=${id}`);
...
//             className='h-[300px] mx-auto rounded-xl w-[300px] bg-white border-dashed border-2 border-gray-600 hover:cursor-pointer'
//           />
//           <FaUpload
//             size={50}
...
const serverAddress = "192.168.18.84:8188";
    ...
    <div>
      {Object.entries(images).map(([nodeId, imagesData]) =>
        imagesData.map((imageData, index) => (
          <img key={`${nodeId}-${index}`} src={`data:image/jpeg;base64,${imageData}`} alt="Generated" />
        ))
      )}
    </div>
  );
}
export default AnimatePrompt;